...
nodo* borrarNodoV2(nodo* lista, int valor, int  * flag) { ///Con un entero
    ...
    if (lista == NULL) {
        return lista; // La lista está vacía, no hay nada que borrar.
    }
...
void mostrarElementosPosicionesParesRecursiva(nodo* nodo, int indice) {
    ...
    // Verificar si el índice es par y mostrar el elemento
    if (indice % 2 == 0) {
        printf(" -%d- ", nodo->dato);
    }

    // Llamar recursivamente con el siguiente nodo y un índice incrementado
...
nodo* invertirListaRecursiva(nodo* lista) {
    if (lista == NULL || lista->siguiente == NULL) {
        return lista; // La lista está vacía o tiene un solo nodo, no es necesario invertirla
    }
    ...
    // Cambiar el vínculo del segundo nodo para que apunte al primero
    segundoNodo->siguiente = lista;

    // Establecer el vínculo del primer nodo como NULL para que sea el último
